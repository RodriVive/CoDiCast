...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        # Define beta (linear variance schedule) 
        self.betas = betas = np.linspace(beta_start, beta_end, timesteps, dtype=np.float64)  # linear variance schedule
        # self.betas = betas = np.linspace(beta_start ** 0.5, beta_end ** 0.5, timesteps, dtype=np.float64) ** 2  # quadratic variance schedule
        ...
        # Calculations for posterior q(x_{t-1} | x_t, x_0), see Eq. (7) in paper
        ...
        # Log calculation clipped because posterior variance is 0 at the beginning of diffusion chain
    ...
    def q_mean_variance(self, x_start, t):
        """Extracts mean & variance at current timestep. Eq. (4) in paper
    ...
    def q_sample(self, x_start, t, noise):
        """Diffuse the data: add noise to image.
